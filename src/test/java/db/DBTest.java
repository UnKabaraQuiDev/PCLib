...
public class DBTest {
	...
	public void test() throws Throwable {
		...
		final TestDB db = new TestDB(ConfigLoader.loadFromJSONFile(new DataBaseConnector(),
				new File("./src/test/resources/config/db_connector.json")));
		 ...
		 * CustomerData.ALL).getPreparedQuerySQL(customers));
		 * System.out.println(((SQLQuery)
		 * CustomerData.ALL_LIMIT.apply(24)).getPreparedQuerySQL(customers));
		 * System.out.println(((SQLQuery)
		 * CustomerData.ALL_OFFSET.apply(12)).getPreparedQuerySQL(customers));
		 * System.out.println(((SQLQuery) CustomerData.ALL_OFFSET_LIMIT.apply(12,
		 * 24)).getPreparedQuerySQL(customers));
		 */
		...
		final long id = customers.query(CustomerData.byNameAndEmail("name1", "email1"))
				.thenApply(PCUtils.first(cd -> cd.getId(), -1L)).runThrow();
		 ...
		 * customers.query(CustomerData.BY_NAME.apply("name1")).runThrow());
		 * System.out.println( "name & email: " +
		 * customers.query(CustomerData.BY_NAME_AND_EMAIL.apply("name1",
		 * "email1")).runThrow()); System.out.println("email: " +
		 * customers.query(CustomerData.BY_EMAIL.apply("email1")).runThrow());
		 */
		 ...
		 * 
		 * System.out.println(orders.query(OrderData.BY_CUSTOMER_ID.apply(customer.getId
		 * ())).runThrow());
		 */
